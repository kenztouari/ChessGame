...
public class Controleur implements Initializable {
	...
	public static ArrayList<Piece> pieceBoard = new ArrayList<Piece>();
...
// black bishop 
	 ...
	 * @FXML
	 * 
	 * void blackSurrender (MouseEvent event) { try { pieceToPlay = null;
	 * previsions.clear(); pieceBoard.clear(); Stage stage=(Stage)
	 * piece_00.getScene().getWindow();
	 * 
	 * stage.close(); Parent root =
	 * FXMLLoader.load(getClass().getResource("whitewin.fxml")); Scene scene = new
	 * Scene(root); Stage newWindow = new Stage(); newWindow.setTitle("White win");
	 * newWindow.setScene(scene); newWindow.show(); } catch(IOException e ) {
	 * e.printStackTrace(); } }
	 */
	@FXML
	...
	/*
	 * @FXML void whiteSurrender(MouseEvent event) { try{ pieceToPlay = null;
	 * 
	 * previsions.clear(); Stage stage=(Stage) piece_00.getScene().getWindow();
	 * stage.close(); Parent root =
	 * FXMLLoader.load(getClass().getResource("blackwin.fxml")); Scene scene = new
	 * Scene(root); Stage newWindow = new Stage(); newWindow.setTitle("Black win");
	 * newWindow.setScene(scene); newWindow.setResizable(false); newWindow.show(); }
	...
	void showPossibilities(MouseEvent event) {
		...
		if (Piece
				.lookForAPiece(board, getValue(GridPane.getColumnIndex(source)), getValue(GridPane.getRowIndex(source)))
				.getStyle().equals("-fx-background-color : BURLYWOOD")) {

			takePiece((Label) Piece.lookForAPiece(board, getValue(GridPane.getColumnIndex(source)),
					getValue(GridPane.getRowIndex(source))));
			...
			if (previsions.get(i) == piece) {
				...
				if (i + 1 < Controleur.previsions.size() && previsions.get(i + 1) instanceof ImageView) {
					...
					if (win(findControleur(previsions.get(i + 1))) == false) {
						...
						pieceToPlay.Move(board, getValue(GridPane.getColumnIndex(piece)),
								getValue(GridPane.getRowIndex(piece)));
						stopPrevision();
					}
				}
				break;
	...
	public void isChecked() {
		...
		//impossible to get coordinates of king, always retrun 0
//		int bKingPosX = getValue(GridPane.getColumnIndex(blackKing));
//		int bKingPosY = getValue(GridPane.getRowIndex(this.blackKing));
//		int wKingPosX = getValue(GridPane.getColumnIndex(whiteKing));
//		int wKingPosY = getValue(GridPane.getRowIndex(whiteBishop_2));
		...
		for (Piece piece : pieceBoard) {
			Node nodeImage = connectBackFront((Node)piece);
				if (whitePlay && piece.getColor() == "black") {
					ArrayList<Node> possibilities = piece.seePossibilities(board,getValue(GridPane.getColumnIndex(nodeImage)), getValue(GridPane.getRowIndex(nodeImage)));
					for (Node n : possibilities) {
						if (GridPane.getColumnIndex(n).equals(wKingPosX)
								|| GridPane.getColumnIndex(n).equals(wKingPosY)) {
							check = true;
						}
					}
				} else if (!whitePlay && piece.getColor() == "white") {
					ArrayList<Node> possibilities = piece.seePossibilities(board,getValue(GridPane.getColumnIndex(nodeImage)), getValue(GridPane.getRowIndex(nodeImage)));
					for (Node n : possibilities) {
						if (GridPane.getColumnIndex(n).equals(bKingPosX)|| GridPane.getColumnIndex(n).equals(bKingPosY)) {
							check = true;
						}
					}
				}
		...
		if (check) {
			...
			if (whitePlay){
				a.setHeaderText("The white king is under threat");
			} else {
				a.setHeaderText("The black king is under threat");
			}
			a.show();
		}
	}

	public Node connectBackFront(Node node) {
		Integer columnIndex = getValue(GridPane.getColumnIndex(node));
		Integer rowIndex = getValue(GridPane.getRowIndex(node));
		
		for (Node n : board.getChildren()) {
			if (columnIndex == getValue(GridPane.getColumnIndex(n))
					&& rowIndex == getValue(GridPane.getRowIndex(n))) 
			{
				return n;
			}
		}
	...
	public void stopPrevision() {
		for (int i = 0; i < Controleur.previsions.size(); i++) {
			if (previsions.get(i) instanceof Label) {
				if ((getValue(GridPane.getColumnIndex(previsions.get(i))) % 2 == 0
						&& getValue(GridPane.getRowIndex(previsions.get(i))) % 2 != 0)
						|| (getValue(GridPane.getColumnIndex(previsions.get(i))) % 2 != 0
								&& getValue(GridPane.getRowIndex(previsions.get(i))) % 2 == 0)) {
